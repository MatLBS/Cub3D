...
/*   Updated: 2024/10/10 16:34:26 by matle-br         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*check_xpm(t_data *data, int i)
{
	...
	if (ft_strncmp(str2, ".xpm", 4) != 0)
		return (printf("No .xpm at end of file.\n"), free(new_str), ft_free_data(data), get_next_line(data->fd, 1), exit(EXIT_FAILURE),  NULL);
...
int	check_rgb(t_data *data, int i)
{
	...
	{
		printf("Bad rgb input.\n");
		return (get_next_line(data->fd, 1), free(new_str), ft_free_data(data), exit(EXIT_FAILURE), 0);
	}
	tab = ft_split(new_str, ',');
	if (!tab)
		return (get_next_line(data->fd, 1), ft_free_data(data), 1);
	nb = check_rgb_suite(tab, data, new_str);
	free(new_str);
	data->map->nb_params += 1;
	return (nb);
}
