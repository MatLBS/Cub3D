...
/*   Updated: 2024/10/24 18:16:51 by matle-br         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_texture_color_suite(t_data *data, int pixelX, int y)
{
	if (data->wall->side == 0 && data->player->raydirx < 0)
		data->player->texx = data->tab_img[data->wall->face].width - data->player->texx - 1;
	if (data->wall->side == 1 && data->player->raydiry > 0)
		data->player->texx = data->tab_img[data->wall->face].width - data->player->texx - 1;
	data->player->step = 1.0 * data->tab_img[data->wall->face].height / data->wall->lineheight;
	data->player->texpos = (data->wall->drawstart - data->height / 2 + data->wall->lineheight / 2) * data->player->step;
	while (y < data->wall->drawend)
	{
		data->player->texy = (int)data->player->texpos % (data->tab_img[data->wall->face].height - 1);
		data->player->texpos += data->player->step;
		data->wall->color = get_pixel_color(&data->tab_img[data->wall->face], data->player->texx, data->player->texy, data);
...
void	get_texture_color(t_data *data, int pixelX)
{
	...
	if (data->wall->hit_door == 1)
		data->wall->face = DOOR;
	// if (data->wall->hit_zombie == 1)
	// 	data->wall->face = FRAME_6;
	...
	if (data->wall->side == 0)
		data->player->wallx = data->player->posy + (data->wall->perpwalldist * data->player->raydiry);
	else
		data->player->wallx = data->player->posx + (data->wall->perpwalldist * data->player->raydirx);
	data->player->wallx -= floor(data->player->wallx);
	data->player->texx = (int)(data->player->wallx * (double)(data->tab_img[data->wall->face].width));
